...
for j in range(0,len(gainSources)):
    
    freq_bin = [190,200,210,220,230,240,250,260,270,280,290,300,310,320,330,340,350,360,370]
    ...
    for i in range(len(freq_bin)-1):
        ...
        select = np.where((sigStruct['souName']==gainSources[j]) & (freq_bin[i] <= sigStruct['Gunn_LO_rx1']) & (sigStruct['Gunn_LO_rx1'] < freq_bin[i+1]))
        if len(select[0]) > 0 :
            print(len(select))
            pp(sigStruct['Gunn_LO_rx1'][select])
            cal_sigma[i] = sum([(x**-2) for x in sigStruct['Avg_sigma_rx1'][select]])**-0.5

    print(cal_sigma, gainSources[j])
